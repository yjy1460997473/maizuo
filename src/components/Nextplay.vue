<template>
	<div class="nextplay" >
		<ul>
			<router-link v-for="(item,index) in movienext"
			:key="index"
			...
			tag="li">
				...
				<div class="next"><span>预购</span></div>
			</router-link>
		</ul>
	</div>
</template>
...
<script>
	...
	export default {
		...
		created: function()  {
			axios.get('../../static/json/nextplay.json').then(response =>{
				this.movienext=response.data.data.films;
